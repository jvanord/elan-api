...
/*
	...
	This is the client-side application file for Book module features in this demo. 
	Include this file on Book application pages after the main application script file ("demo.js") but before any other application scripts.
...
// This function gets a single Book product entity using the Élan product ID
// The second parameter allows the calling script to define actions after a result has been returned
function app_getProduct(productid, callback) {
	var token = app_getToken();
	if (!token) throw new Error('Access Token not found or corrupted.');
	$.ajax({
		method: 'GET',
		url: API_BASE_URI + 'book/product/' + productid,
