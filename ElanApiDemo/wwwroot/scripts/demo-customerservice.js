...
/*
	...
	This is the client-side application file for Customer Service features in this demo. 
...
// This function performs a search for indviduals (but not companies) 
// The first parameter is a collection of search parameters (see documentation or sample code)
// The second parameter allows the calling script to define actions after a result has been returned
function app_findCustomer(searchData, callback) {
	var token = app_getToken();
	if (!token) throw new Error('Access Token not found or corrupted.');
	$.ajax({
		method: 'GET',
		url: API_BASE_URI + 'customerservice/find',
		data: searchData,
